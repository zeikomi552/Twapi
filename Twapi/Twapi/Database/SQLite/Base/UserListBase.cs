...
namespace Twapi.Database.SQLite.Base
{
	/// <summary>
	/// フォローリスト
	/// UserListテーブルをベースに作成しています
	/// 作成日：2021/12/18 作成者gohya
	/// </summary>
	[Table("UserList")]
	public class UserListBase : INotifyPropertyChanged
	{
		#region パラメータ
		...
		public UserListBase(UserListBase item)
		{
			// 要素のコピー
			Copy(item);
		}
		...
		/// </summary>
		/// <param name="item">コピー内容</param>
		public void Copy(UserListBase item)
		{
		...
		public static void Insert(UserListBase item)
		{
		...
		/// <param name="item">Insertする要素</param>
		public static void Insert(SQLiteDataContext db, UserListBase item)
		{
			// Insert
			db.Add<UserListBase>(item);
		}
		#endregion
		...
		public static void Update(UserListBase pk_item, UserListBase update_item)
		{
			using (var db = new SQLiteDataContext())
			{
		...
		/// <param name="pk_item">更新する主キー（主キーの値のみ入っていれば良い）</param>
		/// <param name="update_item">テーブル更新後の状態</param>
		public static void Update(SQLiteDataContext db, UserListBase pk_item, UserListBase update_item)
		{
			var item = db.DbSet_UserList.SingleOrDefault(x => x.UserId.Equals(pk_item.UserId));
		...
		public static void Delete(UserListBase pk_item)
		{
			using (var db = new SQLiteDataContext())
			{
				Delete(db, pk_item);
		...
		/// <param name="db">SQLiteDataContext</param>
		/// <param name="pk_item">削除する主キー（主キーの値のみ入っていれば良い）</param>
		public static void Delete(SQLiteDataContext db, UserListBase pk_item)
		{
			var item = db.DbSet_UserList.SingleOrDefault(x => x.UserId.Equals(pk_item.UserId));
			if (item != null)
			{
				db.DbSet_UserList.Remove(item);
			}
		}
		#endregion
		...
		public static List<UserListBase> Select()
		{
			using (var db = new SQLiteDataContext())
			{
				return Select(db);
		...
		/// <param name="db">SQLiteDataContext</param>
		/// <returns>全件取得</returns>
		public static List<UserListBase> Select(SQLiteDataContext db)
		{
			return db.DbSet_UserList.ToList<UserListBase>();
		}
		#endregion
		#endregion
